...
public record RecipeFilterData(Mode mode, List<ResourceLocation> whitelist, List<ResourceLocation> blacklist) {
    ...
    public boolean allows(ResourceLocation recipeId) {
        return mode == Mode.WHITELIST ? whitelist.contains(recipeId) : !blacklist.contains(recipeId);
    }
    ...
    public RecipeFilterData toggle(ResourceLocation recipeId) {
        List<ResourceLocation> changed = new ArrayList<>(selectedRecipes());
        if (!changed.remove(recipeId)) {
            changed.add(recipeId);
        }
        changed.sort(ResourceLocation::compareTo);
